...
function affichageImagettes(imagettes) {
  ...
  items.forEach(item => {
    // blocage du drag&drop par défaut
    item.addEventListener('dragstart', function( event ) { event.preventDefault(); });
    
    // DRAG&DROP FAIT MAIN pas achevé ...
    item.addEventListener('mousedown', function() { down(item) })
...
//               ---   DRAG&DROP FAIT MAIN non achevé  ---               //
...
function down(item) {
  if (Movable(item, tabImage) != 0) {
    ...
    // AJOUTER LE DRAG&DROP FAIT MAIN !!!!!!
    let Dxf = 0;
    let Dyf = 0;
    //Xo = event.clientX;
    //Yo = event.clientY;
    Xo = item.getBoundingClientRect().x; // Coordonnée x de la case cliquée
    Yo = item.getBoundingClientRect().y; // Coordonnée y de la case cliquée
    _test = function(event) { moveDragDrop(event, item, Dxf, Dyf) }
    item.addEventListener('mousemove', _test);
...
function up(item) {
  Dxf = Dx;
  Dyf = Dy;
  item.removeEventListener('mousemove', _test);
}

function moveDragDrop(event, item, Dxf, Dyf) {
  var div_special = getItemByValue(tabImage, "special"); // La div_special est la case vide
  var x_special = div_special.getBoundingClientRect().x; // Coordonnée x de la case vide
  var y_special = div_special.getBoundingClientRect().y; // Coordonnée y de la case vide
  var long = 88;
  ...
  if (Movable(item, tabImage) == 1) { // cas où la case cliquée et la case vide sont sur la même ligne
    var diffX = Math.floor(Math.abs(x_special-Xo) / long) // écart sur la ligne entre les cases
    if (Xo < x_special) { // case cliquée à gauche de la case blanche
      var j = Number(item.id[7])
      for (let k = j ; k<= j+diffX-1 ; k++) {
        var it = document.getElementById(item.id.substr(0, 7)+k);
        it.style.transform = `translate(${Dx}px,${Dy}px)`;
      }
    }
    if (x_special < Xo) { // case cliquée à droite de la case blanche
      var j = Number(item.id[7])
      for (let k = j ; k>= j-diffX+1 ; k--) {
        var it = document.getElementById(item.id.substr(0, 7)+k);
        it.style.transform = `translate(${Dx}px,${Dy}px)`;
  ...
  if (Movable(item, tabImage) == 2) { // cas où la case cliquée et la case vide sont sur la même colonne
    var diffY = Math.floor(Math.abs(y_special-Yo) / long) // écart sur la colonne entre les cases
    if (Yo < y_special) { // case cliquée au dessus de la case blanche
      var i = Number(item.id[5]) // colonne de l'item cliqué
      for (let k = i ; k<= i+diffY-1 ; k++) {
        var it = document.getElementById(item.id.substr(0, 5) + k + item.id.substr(6));
        it.style.transform = `translate(${Dx}px,${Dy}px)`;
      }
    }
    if (y_special < Yo) { // case cliquée en dessous de la case blanche
      var i = Number(item.id[5]) // colonne de l'item cliqué
      for (let k = i ; k>= i-diffY+1 ; k--) {
        var it = document.getElementById(item.id.substr(0, 5) + k + item.id.substr(6));
        it.style.transform = `translate(${Dx}px,${Dy}px)`;
